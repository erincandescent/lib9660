...
{
    if (!f->fs->read_sector(f->fs, f->buf, f->first_sector + f->position / 2048))
        return L9660_EIO;
    else
        return L9660_OK;
}
...
l9660_status l9660_read(l9660_file *f, void* buf, size_t size, size_t *read)
{
    ...
    memcpy(buf, f->buf + fsectoff(f), size);
